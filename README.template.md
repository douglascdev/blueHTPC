...
1. Modify `disk_config/iso-kde.toml` (or `disk_config/iso-gnome.toml`) to point to your custom container image in the kickstart `bootc switch` line, substituting your registry, organization, and image name.
2. `IMAGE_REGISTRY`, `IMAGE_NAME` and `DEFAULT_TAG` in `build-disk.yml` are auto-derived from your repository — no edits needed.
